...
Getting Semantius **[Agent Skills](/docs/reference/agent-skills)** running takes three steps: install the Semantius CLI, set up credentials, then add the skills to your agent.
...
The CLI needs two values to talk to your platform: an API key and an organization name. The easiest way to get both is to ask your Ops MCP for them. Make sure the [Ops MCP connector is installed first](/docs/reference/mcp-connectors/installation), then send this prompt to your agent:
...
- **[Create a Model →](/docs/reference/models/create)**: build a custom semantic model from scratch with the Business Analyst skill.
- **[Templates →](/docs/reference/models/templates)**: browse curated, ready-to-use models you can deploy as-is or customize first.
- **[Deploy →](/docs/reference/models/deploy)**: ship a model file to a live Semantius instance.
- **[Optimize →](/docs/reference/models/optimize)**: pull a live module back into a markdown spec to audit and refine.
