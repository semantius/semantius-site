...
A **semantic model** is the heart of every Semantius platform. It describes your domain as data: the entities you care about, the attributes that define them, the relationships between them, and the rules that govern who can access what. Models are built and evolved through [agent skills](/docs/reference/agent-skills) and consumed by AI clients through [MCP connectors](/docs/reference/mcp-connectors).
...
For a concrete example, see the [SaaS Expense Tracker model](/models/saas_expense_tracker/model). It is fairly detailed (a real model usually has dozens of fields and a non-trivial relationship graph), but in day-to-day work you rarely open the file by hand: the [Create](/docs/reference/models/create), [Customize](/docs/reference/models/create#customize-or-extend-an-existing-model), [Deploy](/docs/reference/models/deploy), and [Optimize](/docs/reference/models/optimize) skills do the reading, writing, and diffing for you.
...
- [Create](/docs/reference/models/create) a custom model from scratch by interviewing your domain.
- [Templates](/docs/reference/models/templates) are curated, ready-to-use models you can deploy as-is or customize first.
- [Deploy](/docs/reference/models/deploy) a model to a live Semantius instance, with safe diffing and conflict resolution.
- [Optimize](/docs/reference/models/optimize) by reverse-engineering a live module back into a markdown spec you can audit and improve.
