...
Once the [CLI is installed](/docs/reference/agent-skills/installation#1-install-the-semantius-cli) and credentials are in place, every interaction with Semantius from the shell goes through a single binary: `semantius`. This page walks through the basics so you can get productive on your own.
...
The first command prints the version. The second connects to your platform and lists the modes that are wired up. If either fails, revisit the [installation page](/docs/reference/agent-skills/installation) before going further.
...
- **[use-semantius skill](/docs/reference/cli/use-semantius)**: install a ready-made skill that teaches a coding agent to drive the CLI productively, including the schema-first creation order and read-before-write rules.
- **[Agent Skills Overview](/docs/reference/agent-skills)**: see how the higher-level skills (Business Analyst, Schema Editor, RBAC Configurator, Deployer, Optimizer) sit on top of this same CLI.
