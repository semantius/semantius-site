...
`use-semantius` is an **agent skill** that ships alongside the [Semantius CLI](/docs/reference/cli). Where the CLI gives an agent the ability to call Semantius, this skill teaches the agent **how to use it well**: which layer to reach for, which tool to pick, and which mistakes to avoid.
...
- **[CLI Overview](/docs/reference/cli)**: why the CLI exists and where it fits.
- **[The semantius command](/docs/reference/cli/command)**: the underlying surface this skill drives.
- **[Agent Skills Overview](/docs/reference/agent-skills)**: the broader set of skills (Business Analyst, Schema Editor, RBAC Configurator, Deployer, Optimizer) that build on `use-semantius`.
