...
/**
 * Shown inside the search modal when /pagefind/pagefind.js cannot be loaded.
 * That only happens in `astro dev` before any build has produced an index
 * (the dev server serves /pagefind/ from the last build output).
 */
export default function DevSearchModal({ onClose, labels }) {
  return (
    <div className="p-4 sm:p-8 text-center flex flex-col items-center gap-6">
      ...
      <div className="space-y-2">
        <h3 className="text-2xl font-bold tracking-tight">{labels?.title || "Search needs a build first"}</h3>
        <p className="text-foreground/60 max-w-sm mx-auto leading-relaxed">
          {labels?.description || "Pagefind indexes the site at build time and the dev server serves the index from the last build. Run this once, then reload:"}
        </p>
      </div>
      ...
      <div className="w-full bg-foreground/5 p-4 rounded-xl border border-foreground/5 font-mono text-sm relative group overflow-hidden">
        ...
        <code className="relative z-10 text-primary font-bold">pnpm build</code>
      </div>
